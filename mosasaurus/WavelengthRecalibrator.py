...
def subtractcontinuum(y):
    ...
    This was tuned by hand for use with the cross-correlation.
    It shouldn't be treated too much as a general-purpose tool.

    Parameters
    ----------

    y : array
        Any array of y-values.

    Returns
    -------
    subtracted : array
        A continuum subtracted array, normalized to its standard deviation.
    '''
...
class WavelengthRecalibrator(Talker):
    ...
    to determine the stretches needed to line up all its spectra.
    Wavelength shifts and stretches are estimated by cross-correlating
    spectra (across both stars and time) in specified "alignmentranges",
    ...
    def __init__(self, cube, outsidecube='', visualize=True):
        ...
        cube : a mosasaurus.Cube object
            A cube of spectra, spanning multiple stars and multiple times.

        visualize : bool
            Should we save plots of the individual correlation functions
            and estimated wavelength shifts for each spectrum?
        '''
    ...
    def save(self):
        ...
        self.speak(
            'exported the spectral shifts and stretches to {}'.format(self.filename))
    ...
    def determineStretches(self, plot=True):
        ...
        self.corrections['midpoint'] = np.mean(
        ...
        for i, prefix in enumerate(self.corrections['prefixes']):
            ...
            for iline, line in enumerate(alignmentranges.keys()):
                ...
                left = np.argmin(
                    np.abs(self.unshiftedcube.spectral['wavelength'] - l))
                right = np.argmin(
                    np.abs(self.unshiftedcube.spectral['wavelength'] - r))
                ...
                for istar, star in enumerate(self.unshiftedcube.stars):
                    ...
                    self.corrections['offset_{}'.format(
                        line)][star][prefix] = offset
                    ...
                    if self.visualize:
                        ...
                        ax.plot(wave, start / np.std(start),
                                color='gray', linewidth=3, alpha=0.4)
                        ax.plot(wave, this / np.std(this),
                                color=self.unshiftedcube.starcolor(star))
                        if iline == 0:
                            ax.set_ylabel(
                                '{}/{}'.format(i + 1, self.unshiftedcube.numberoftimes))
                        ...
                        ax.set_autoscaley_on(True)
                        ax.plot(x - len(wave) / 2 + 1, xc,
                                alpha=0.5, color='gray')
                        plt.setp(ax.get_yticklabels(), visible=False)
                        ax.scatter(peak - len(wave) / 2 + 1,
                                   fit(peak), color='gray')
                        # ax.set_xlim(0,len(x))
                        if iline == 0:
                        ...
                        ax.axvline(len(x) / 2.0 - len(wave) / 2, color='gray',
            ...
            for star in localshifts.keys():
                ...
                self.speak(
                    'stretch for {}+{} is {}'.format(star, prefix, phrase))

                if self.visualize:
                    color = self.unshiftedcube.starcolor(star)
                    plt.plot(
                        self.unshiftedcube.spectral['wavelength'], dw, color=color)
                    plt.scatter(linecenters, offsets, color=color)

            if self.visualize:
                plt.xlim(np.min(self.unshiftedcube.spectral['wavelength']), np.max(
                    self.unshiftedcube.spectral['wavelength']))
                ...
                mkdir(pltdir)
                pltfilename = os.path.join(pltdir, 'shift_{}.pdf'.format(
                    self.unshiftedcube.obs.fileprefixes['science'][i]))
    ...
    def recreateSupersampled(self):
        ...
        for star in self.unshiftedcube.stars:
            ...
            for prefix in self.corrections['prefixes']:
                extractedFilename = os.path.join(
                ...
                self.speak(
                    'nudge wavelengths for {} by {}'.format(prefix, phrase))
                extracted['wavelength'] = wavelength

                # save the new shifted spectrum out to a file
                shiftedExtractedFilename = os.path.join(
                    directory, 'shiftedextracted_{}.npy'.format(prefix))
