...
class Calibration(Talker):
    ...
    def createMasterImage(self, imageType=None, remake=False):
        ...
        if (imageType == "dark") & (imageType not in self.obs.fileprefixes.keys()):
            ...
            masterFilePrefix = os.path.join(
                self.calibrationDirectory, "master_{0}".format(imageType)
            )
            noisestring = "StdDev"

            self.images[imageType] = np.array([0.0])
            self.images[imageType + noisestring] = np.array([0.0])

            writeFitsData(self.images[imageType], masterFilePrefix + ".fits")
            writeFitsData(
                self.images[imageType + noisestring],
                masterFilePrefix + noisestring + ".fits",
            )
            return
        ...
        try:
            ...
            self.images[imageType + noisestring] = readFitsData(
                masterFilePrefix + noisestring + ".fits"
            )
            ...
            self.speak(
                ...
                + truncate(str(self.obs.fileprefixes[imageType]), n=30)
            )
            self.images[imageType], self.images[imageType + noisestring] = (
                self.createStackedImage(
                    self.obs.fileprefixes[imageType], imageType=imageType
                )
            )
    ...
    def createStackedImage(self, n, imageType=None, threshold=5.0):
        ...
        if imageType == "science":
            ...
            truncation = self.obs.instrument.maximumimagesinmemoryforscience

            # if we're trying to load too many images, skip some of them
            stride = int(np.maximum(len(n) / truncation, 1))
            if stride > 1:
                self.speak(
                    "stacking {0}/{2} {1} images".format(len(n), imageType, truncation)
                )
            else:
            ...
            loadlimit = self.obs.instrument.maximumimagesinmemory

            # if we're trying to load too many images, skip some of them
            stride = int(np.maximum(len(n) / loadlimit, 1))
            if stride > 1:
                self.speak(
                    "stacking {0}/{2} {1} images".format(len(n), imageType, loadlimit)
                )
            else:
                self.speak("stacking {0} {1} images".format(len(n), imageType))

            # create a 3D array of images
            array = self.ccd.loadImages(n[:loadlimit], imageType=imageType)
        ...
        mean, noise = twod.stack(array, axis=0, threshold=threshold)
    ...
    def createBadPixelMask(self, visualize=True):
        ...
        badPixelFilename = os.path.join(
            self.calibrationDirectory, "master_badpixels.fits"
        )
        try:
            ...
            if "flat" in self.obs.fileprefixes.keys():
                ...
                noise = np.median(
                    np.abs(cube - median.reshape(1, cube.shape[1], cube.shape[2])), 0
                )
                ...
                ax.set_ylabel("RMS")
                self.images["badpixels"] = bad.astype(int)
                if visualize:
                    self.display.one(self.images["badpixels"])
                    answer = self.input(
                        "Does the bad pixel mask seem reasonable? [Y,n]"
                    ).lower()
                assert "n" not in answer

            else:
                self.images["badpixels"] = np.zeros_like(self.bias())
