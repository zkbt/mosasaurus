...
class Trace(Talker):
    ...
    def __init__(self, aperture):
        ...
        try:
            ...
            inner = (
                np.min(self.extractionwidths)
                + self.instrument.extractiondefaults["skyGap"]
            )
            outer = inner + self.instrument.extractiondefaults["skyWidth"]
            self.skyoffsets = [
                dict(top=outer, bottom=inner, whattodo=1),
    ...
    def filename(self):
        return os.path.join(
            self.aperture.directory, "trace_{0}.npy".format(self.aperture.name)
        )
    ...
    def save(self):
        ...
        np.save(self.filename, dict(coef=self.tracefitcoef, width=self.tracefitwidth))
        self.speak("saved trace parameters to {0}".format(self.filename))

        # save the extraction and sky masks
        filename = os.path.join(
            self.aperture.directory,
            "extractionmasks_{0}.npy".format(self.aperture.name),
        )
        np.save(
            filename,
            dict(skyoffsets=self.skyoffsets, extractionwidths=self.extractionwidths),
        )
        ...
        filename = os.path.join(
            self.aperture.directory,
            "tracedefinition_{0}.pdf".format(self.aperture.name),
        )
        self.figure.savefig(filename)

        # save skyMask and extractionMask for each width
        filename = os.path.join(
            self.aperture.directory, "masks_{0}.npy".format(self.aperture.name)
        )
        masks = {}
    ...
    def load(self):
        ...
        trace_parameters = np.load(self.filename, allow_pickle=True)[()]
        self.tracefitcoef = trace_parameters["coef"]
        self.tracefitwidth = trace_parameters["width"]
        ...
        extraction_parameters = np.load(filename, allow_pickle=True)[()]
        self.skyoffsets = extraction_parameters["skyoffsets"]
        self.extractionwidths = extraction_parameters["extractionwidths"]
        self.speak("saved extraction mask parameters to {0}".format(filename))
        self.numberofapertures = len(self.extractionwidths)
        self.narrowest, self.widest = oned.minmax(self.extractionwidths)
    ...
    def run(self):
        ...
        while self.notconverged:
            ...
            options = {}
            options["w"] = dict(
                description="[w]rite and quit",
                function=self.writeandquit,
                requiresposition=False,
            )
            options["q"] = dict(
                description="[q]uit without writing",
                function=self.quit,
                requiresposition=False,
            )
            options["c"] = dict(
                description="move the [c]rosshair, and plot slices along it",
                function=self.moveCrosshair,
                requiresposition=True,
            )
            options["t"] = dict(
                description="add a guess for a [t]race point",
                function=self.addTracePoint,
                requiresposition=True,
            )
            options["e"] = dict(
                description="[e]xtend a sky region (twice for start and stop)",
                function=self.modifySky,
                requiresposition=True,
            )
            options["r"] = dict(
                description="[r]emove a sky region (twice for start and stop)",
                function=self.modifySky,
                requiresposition=True,
            )
            options["f"] = dict(
                description="[f]it the trace, using the star's centroids and sky areas",
                function=self.fitTrace,
                requiresposition=False,
            )
            options["s"] = dict(
                description="[s]et the [s]ize of the [s]tar's smallest extraction region",
    ...
    def setSizes(self, pressed=None, default=False):
        ...
        if default:
            ...
            self.numberofapertures = self.instrument.extractiondefaults[
                "numberofapertures"
            ]
        else:
            self.speak("Please redefine the aperture sizes.")
            self.speak(
                " (The old aperture sizes were {}.)".format(self.extractionwidths)
            )
            self.speak(' ["d" for any of these for defaults]')

            try:
                self.narrowest = float(self.input("What is the narrowest aperture?"))
            except ValueError:
                self.narrowest = self.instrument.extractiondefaults["narrowest"]

            try:
                self.widest = float(self.input("What is the widest aperture?"))
            except ValueError:
                self.widest = self.instrument.extractiondefaults["widest"]

            try:
                self.numberofapertures = int(
                    self.input("How many apertures do you want?")
                )
            except ValueError:
                self.numberofapertures = self.instrument.extractiondefaults[
                    "numberofapertures"
        ...
        self.extractionwidths = np.linspace(
            self.narrowest, self.widest, self.numberofapertures
        )
    ...
    def modifySky(self, pressed, whattodo=None):
        ...
        if len(self.skykeys) == 2:
            ...
            if a != b:
                self.speak(
                    "Uh-oh, you pressed {0} and then {1}, "
            ...
            d = {"top": offsets.max(), "bottom": offsets.min(), "whattodo": whattodo}
            ...
            self.speak(
                'Please do another "{}" again to edit the sky.'.format(pressed.key)
            )
        else:
    ...
    def updateMasks(self):
        ...
        for line in self.plotted["extractionedges"]:
            line.remove()
        offsets = (
            np.array([-1, 1])[np.newaxis, :] * self.extractionwidths[:, np.newaxis]
        ).flatten()
        x = self.waxis
        y = self.traceCenter(x)[:, np.newaxis] + offsets[np.newaxis, :]
        self.plotted["extractionedges"] = self.ax["2d"].plot(
            x, y, linewidth=1, alpha=0.5, color="darkgreen", zorder=1000
        )
    ...
    def addTracePoint(self, pressed):
        ...
        # update the plot
        self.plotted["tracepoints"].set_data(
            self.tracepoints["w"], self.tracepoints["s"]
        )

        # update the guess
        order = np.minimum(len(self.tracepoints["w"]) - 1, self.order)
        self.traceguesscoef = np.polyfit(
            self.tracepoints["w"], self.tracepoints["s"], order
        )
    ...
    def fitTrace(self, *args):
        ...
        weights = np.maximum(
            fine * considerstar * self.aperture.images["Subtracted"], 0
        )
        weights += 1.0 / np.sum(weights)

        # use the roughly subtracted image to fit centroids
        flux = np.average(
            self.aperture.images["Subtracted"],
            axis=self.aperture.sindex,
            weights=weights,
        )
        ...
        try:
            fluxWeightedCentroids = np.ma.average(
                self.aperture.s,
                axis=self.aperture.sindex,
        ...
        self.plotted["fittedwidthsup"].set_data(
            self.waxis, fluxWeightedCentroids + fluxWeightedWidths
        )
        self.plotted["fittedwidthsdown"].set_data(
            self.waxis, fluxWeightedCentroids - fluxWeightedWidths
        )

        # fit a polynomial to the ridge of the spectrum
        self.tracefitcoef = np.polyfit(
            self.waxis,
            fluxWeightedCentroids,
            self.order,
            w=flux / fluxWeightedWidths**2,
        )
    ...
    def setup(self, percentiles=[1, 99]):
        ...
        plt.ion()
        self.figure = plt.figure("tracing the spectrum", figsize=(8, 4), dpi=100)
        ...
        self.plotted["tracepoints"] = self.ax["2d"].plot(
            [], [], color="seagreen", linewidth=0, marker="o"
        )[0]

        # add the sky subtraction mask
        self.plotted["skymask"] = self.ax["2d"].imshow(
            self.skymaskimage,
            cmap=one2another(
        ...
        y = self.traceCenter(x)[:, np.newaxis] + offsets[np.newaxis, :]
        self.plotted["extractionedges"] = self.ax["2d"].plot(
            x, y, linewidth=1, alpha=0.25, color="turquoise", zorder=1000
        )
        ...
        self.plotted["crosss"] = self.ax["2d"].axvline(self.crosshair["w"], **crosskw)
        self.plotted["crosssextend"] = self.ax["slicew"].axvline(
            self.crosshair["w"], linestyle="--", **crosskw
        )
        self.plotted["crossw"] = self.ax["2d"].axhline(self.crosshair["s"], **crosskw)
        self.plotted["crosswextend"] = self.ax["slices"].axhline(
            self.crosshair["s"], linestyle="--", **crosskw
        )
        # plot slices
        slicekw = dict(color="darkorange", linewidth=1, alpha=1)
        self.plotted["slices"] = self.ax["slices"].plot(*self.slices, **slicekw)[0]
        self.plotted["slicew"] = self.ax["slicew"].plot(*self.slicew, **slicekw)[0]

        # plot the recently dragged regions
        self.plotted["dragger"] = self.ax["2d"].plot(
            [], [], marker="+", alpha=0.25, color="royalblue"
        )[0]

        # plot the estimated centroids and widths
        for x in ["up", "down"]:
            self.plotted["fittedwidths{0}".format(x)] = self.ax["2d"].plot(
                [], [], linewidth=1, color="mediumorchid", alpha=1.0
            )[0]
    ...
    def labelWidths(self):
        self.plotted["widthlabels"] = [
            self.ax["2d"].text(
                (i + 0.5) / float(self.numberofapertures),
                0.1,
    ...
    def skymask(self, width):
        ...
        absolutedistancefromtrace = np.abs(
            self.aperture.s - self.traceCenter(self.aperture.w)
        )
        mask[
            absolutedistancefromtrace
            < (width + self.instrument.extractiondefaults["skyGap"])
        ] = 0
        # self.speak('')
    ...
    def skymaskimage(self):
        ...
        for i, width in enumerate(self.extractionwidths):
            ...
            image[:, int(i * chunksize) :] = mask[:, int(i * chunksize) :] * (
                1.0 - 0.2 * (i % 2)
            )
