...
class CubeVisualizer(Talker):
    ...
    def explore(
        self, image=None, key="raw_counts", star=None, vmin=None, vmax=None, **kw
    ):
        """
    ...
    def overlayQuality(self, color="tomato"):
        ...
        """
        cmap = one2another(color, color, alphabottom=1.0, alphatop=0.0)
        a = self.loupe.ax["2d"]
    ...
    def makeSliceMovies(
        self, keys=["wavelengthed", "corrected", "raw_counts"], remake=False, stride=1
    ):
        """
        ...
        for key in keys:
            ...
            self.loupe.movieSlice(
                direction="y", filename=filename, remake=remake, stride=stride
            )
