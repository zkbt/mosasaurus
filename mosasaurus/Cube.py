...
starcm = one2another("royalblue", "sienna")
...
class Cube(Talker):
    ...
    def __init__(self, obs, width=None, remake=False, max=None, **kwargs):
        ...
        if width == None:
            self.speak(
                "Yikes! Nobody set an extraction width with which to create this cube!"
            )
            self.listWidths()
            width = float(self.input("What width would you like? [as a number]"))

        self.width = width

    def __repr__(self):
        return "<Cube of {} Spectra for {} Stars for {}>".format(
            self.numberoftimes, self.numberofstars, self.obs
        )
    ...
    def listStars(self):
        ...
        os.system(
            "open {}".format(os.path.join(self.directory, "genericfinderchart.pdf"))
        )
    ...
    def listWidths(self):
        ...
        """
        self.speak(
            "checking widths available for {}".format(
                self.obs.reducer.extractionDirectory
            )
        )
    ...
    def populate(self, remake=False, max=None, visualize=True, shift=True):
        ...
        try:
            ...
            self.speak(
                "the cube for {} has already been loaded".format(
    ...
    def loadSpectra(self, remake=False, visualize=True, max=None):
        ...
        for timepoint in range(self.numberoftimes):
            ...
            for istar, star in enumerate(self.stars):
                ...
                extractedFile = os.path.join(
                    self.starDirectories[istar], "extracted_{0}.npy".format(fileprefix)
                )
                if self.shift:
                    spectrumFile = os.path.join(
                        self.starDirectories[istar],
                        "stretchedsupersampled",
                        "stretchedsupersampled_{0}.npy".format(fileprefix),
                    )
                else:
                    spectrumFile = os.path.join(
                        self.starDirectories[istar],
                        "supersampled",
                        "supersampled_{0}.npy".format(fileprefix),
                    )
                ...
                for key in self.cubekeys + ["ok"]:
                    ...
                    try:
                        ...
                        if key == "ok":
                            self.cubes[key][star] = np.ones(
                                (self.numberoftimes, self.numberofwavelengths)
                            ).astype(bool)
                        else:
                            self.cubes[key][star] = np.zeros(
                                (self.numberoftimes, self.numberofwavelengths)
                            ).astype(float)
                    self.speak(
                        "updating cubes['{key}']['{star}'][{timepoint},:]".format(
                            **locals()
                        )
                    )
                    ...
                    if key != "ok":
                        self.cubes[key][star][timepoint, :] = supersampled[
                            key + "_" + self.widthkey
                        ]
                    ...
                    if "raw_counts" in key:
                        ...
                        self.speak(
                            "(raw_counts sum to {} for {})".format(s, fileprefix)
                        )
                        assert s > 0.0
                ...
                for key in [
                    ...
                    try:
                        ...
                        self.squares[key][star] = np.zeros(self.numberoftimes).astype(
                            float
                        )

                    if key in ["shift", "stretch"]:
                        self.squares[key][star][timepoint] = self.wavelengthstretches[
                            key
                        ][star][fileprefix]
                    else:
                        self.squares[key][star][timepoint] = np.nanmedian(
                            extracted[self.width][key]
                        )
                    self.speak(
                        "updating squares['{key}']['{star}'][{timepoint}] = {value}".format(
                            value=self.squares[key][star][timepoint], **locals()
                        )
                    )
            ...
            self.speak(
                "{0}/{1} spectra loaded into cube".format(timepoint, self.numberoftimes)
            )
        ...
        if truncate:
            self.speak(
                "couldn't find all requested spectra, so truncated cube at a length of {0}".format(
                    timepoint
                )
            )
        ...
        self.temporal["ok"] = np.ones(self.numberoftimes).astype(
            bool
        )  # self.temporal['cosmicdiagnostic'] < self.obs.cosmicAbandon
    ...
    def save(self):
        self.speak(
            "attempting to save the cube of loaded, shifted, compiled spectra to {0}".format(
                self.filename
            )
        )
        tosave = {}
    ...
    def packageandexport(self):
        ...
        toexport = os.path.join(
            base, "cube_" + self.meta["target"] + "_" + self.meta["night"]
        )
        mkdir(toexport)
        commandstorun = []
        ...
        commandstorun.append(
            "cp {} {}/.".format(os.path.join(base, "genericfinderchart.pdf"), toexport)
        )

        # copy the finder chart
        commandstorun.append(
            "cp {} {}/.".format(os.path.join(base, "extractionCenters.txt"), toexport)
        )
        ...
        for s in self.stars:
            ...
            apertureimage = os.path.join(
                base, s, "animatedextraction/formovie_00000.png"
            )
            commandstorun.append(
                "cp {} {}/{}_whichaperture.png".format(apertureimage, stardir, s)
            )
    ...
    def cubeMegaComparison(self):
        ...
        if len(self.comparisons) == 1:
            ...
            raise NameError(
                "Darn it -- the mega-comparison hasn't been implemented yet!"
            )
    ...
    def imageCube(self, keys=None, stars=None, remake=False):
        ...
        for option, description in options.items():
            filename = os.path.join(
                ...
                "{}+{}+{}+{}.pdf".format(
                    option,
                    {True: "shifted", False: "raw"}[self.shift],
                    "-".join(keys),
                    "+".join(stars),
                ),
            )
            ...
            for i, key in enumerate(keys):
                for j, star in enumerate(stars):
                    ...
                    if option == "wavelength":
                        ...
                        self.speak(
                            "displaying the measurements normalized by their median spectrum for {}".format(
                                key
                            )
                        )
                        z = self.cubes[key][star]
                    ...
                    if option == "comparison":
                        ...
                        self.speak(
                            "displaying the measurements divided by the comparison[s] for {}".format(
                                key
                            )
                        )
                    ...
                    self.speak(
                        "the limits for {} on {} are [{} to {}]".format(
                            key, star, vmin, vmax
                        )
                    )
  ...
  def nudgeWavelengths(self):
    ...
    def align_lines(stars, line_range, offset, line_pos, line_poses, plot=False):
        ...
        ####### KLUDGE?????
        # corrmaster = oned.subtractContinuum(corrmaster, n=2) + 0.0
        ...
        for s in range(len(stars)):
            ...
            ####### KLUDGE?????
            # corrstar = oned.subtractContinuum(corrstar) + 0.0
  ...
  """
    ...
    def movieCube(
        ...
        **kw
        ...
        self.writer = matplotlib.animation.FFMpegWriter(
        ...
        if self.shift:
            filename = os.path.join(
                self.directory,
                "shifted_cube_{0}stars_{1}spectra_{2}stride.mp4".format(
                    self.numberofstars, self.numberoftimes, stride
                ),
            )
        else:
            filename = os.path.join(
                self.directory,
                "cube_{0}stars_{1}spectra_{2}stride.mp4".format(
                    self.numberofstars, self.numberoftimes, stride
                ),
            )
    ...
    def plotSpectra(self, which, remake=False, wavelengthscale=10.0, figsize=None):
        ...
        try:
            ...
            gs = plt.matplotlib.gridspec.GridSpec(
                len(self.cubekeys),
                self.numberofstars,
                height_ratios=[1] + [0.5] * (len(self.cubekeys) - 1),
                hspace=0.11,
                wspace=0.2,
                left=0.1,
                right=0.75,
            ...
            gstemporal = plt.matplotlib.gridspec.GridSpec(
                1,
                len(self.globallinekeys) + len(self.starlinekeys),
                left=0.78,
                right=0.98,
                wspace=0.05,
                bottom=0.1,
            ...
            for istar, star in enumerate(self.stars):
                ...
                # if self.widths == None:
                #    self.widths = np.sort([float(x.split('_')[-1].split('px')[0])
                ...
                for i in range(len(self.cubekeys)):
                    ...
                    try:
                        self.ax_spectra[k].append(
                            plt.subplot(gs[i, istar], sharex=sharex)
                        )
                    except:
            ...
            self.ax_spectra[k][0].set_xlim(
                np.min(self.spectral["wavelength"] / wavelengthscale),
                np.max(self.spectral["wavelength"] / wavelengthscale),
            )
            ...
            for i, l in enumerate(self.linekeys):
                ...
                if l in self.globallinekeys:
                    ...
                    for s in self.stars:
                        ...
                        ax.plot(
                            x[ok],
                            np.arange(self.numberoftimes)[ok],
                ...
                self.bars.append(
                    ax.axhline(which, color="gray", linewidth=4, alpha=0.5)
                )
                ...
                if i == int(len(self.linekeys) / 2):
                    this_time = astropy.time.Time(
                        self.temporal["jd_utc"][which], format="jd", scale="utc"
                    ).iso
                    self.timestamp = ax.set_title(
                        "{0}: {1}".format(self.obs.target.name, this_time)
                    )
            ...
            for istar, star in enumerate(self.stars):
                ...
                for k in self.cubekeys:
                    try:
                        ...
                        self.ps[star][k] = self.ax_spectra[k][istar].plot(
                            self.spectral["wavelength"],
                            self.cubes[k][star][which, :],
                            color=self.starcolor(star),
                            **kw
                        )[0]
        ...
        for istar, s in enumerate(self.stars):
            ...
            self.ax_spectra[self.cubekeys[0]][istar].set_title(
                "image {0},\nstar {1}, aperture {2}".format(
                    self.temporal["fileprefix"][which],
                    istar,
                    s.replace("aperture_", ""),
                )
            )
