...
def align_lines(stars, line_range, offset, line_pos, line_poses, plot=False):
    ...
    idxmaster = (starmaster["wavelength"] >= line_range[0]) * (
        starmaster["wavelength"] <= line_range[1]
    )
    ...
    for s in range(len(stars)):
        ...
        idxstar = (stars[s]["wavelength"] >= line_range[0]) * (
            stars[s]["wavelength"] <= line_range[1]
        )
        ...
        for shift in np.linspace(0, arraylen + offset, (arraylen + offset) * 10 + 1):
            ...
            corrarray = (
                corrstar[initpx + startpx : initpx + startpx + arraylen] * therest
                + corrstar[initpx + startpx + 1 : initpx + startpx + arraylen + 1]
                * newbit
            )
            # if shift == 0:
            #    plt.plot(corrmaster)
        ...
        if len(line_poses) < 13:
            while ((firstshift - offset) > 5.0) or ((firstshift - offset) < -5.0):
                ...
                firstshift = firstrange[firstshiftind]
                if (
                    ((firstshift - offset) < 5.0)
                    and ((firstshift - offset) > -5.0)
                    and plot == True
                    and s != len(stars) - 1
                ):
                    plt.plot(corrs)
        ...
        # there is probably a better way to do this... like going through the whole thing and then looking for outliers
            ...
            med = np.median(
                np.array(line_poses)[:, s][-13:] - np.array(line_poses)[:, -1][-13:]
            )
            std = np.std(
                np.array(line_poses)[:, s][-13:] - np.array(line_poses)[:, -1][-13:]
            )
            ...
            if (
                pxtowavemaster(linepx + firstshift - offset) - line
                > (med + timesstd * std)
            ) or (
                pxtowavemaster(linepx + firstshift - offset) - line
                < (med - timesstd * std)
                ...
                corrind = np.where(
                    (
                        pxtowavemaster(linepx + firstrange - offset) - line
                        >= (med - timesstd * std)
                    )
                    & (
                        pxtowavemaster(linepx + firstrange - offset) - line
                ...
                firstshiftind = (
                    np.where(corrsclipped == np.max(corrsclipped))[0][0] + corrind[0][0]
                )
                firstshift = firstrange[firstshiftind]
                if plot == True and s != len(stars) - 1:
                    plt.plot(corrs)
                    plt.axvline(
                        np.where(corrsclipped == np.max(corrsclipped))[0][0]
                        + corrind[0][0]
                    )
                    plt.title(str(line_range))
                    plt.show()
        ...
        parabrange = np.linspace(
            parabinds[0], parabinds[-1], 100 * (len(parabinds) - 1)
        )
        ...
        if s == len(stars) - 1:
            ...
            line_pos.append(newline)
        if plot == True:
            print(finalshift - offset)
        # print newline
...
mastern = "1825"
starmasterstr = (
    "/h/mulan0/data/working/GJ1132b_ut160227_28/multipleapertures/aperture_544_1024/extracted"
    + mastern
    + ".npy"
)
# get the values to recreate the wavelength solution from mosasaurus
coef, domain = np.load(
    "/h/mulan0/data/working/GJ1132b_ut160227_28/multipleapertures/aperture_544_1024/aperture_544_1024_wavelengthcalibration.npy"
)[()]
# 2016_03_03-04
# mastern = '0945'
