...
class WavelengthCalibrator(Talker):
    ...
    def loadWavelengthIdentifications(self, restart=False):
        ...
        try:
            ...
            self.speak(
                "checking for a custom wavelength-to-pixel file: {0}".format(
                    self.waveidfilename
                )
            )
    ...
    def loadCalibration(self):
        ...
        self.speak(
            "loaded wavelength calibration" "from {0}".format(self.calibrationfilename)
        )
    ...
    def findRoughShift(self, blob=2.0):
        ...
        self.waveids[
            "pixel"
        ] += self.aperture.obs.instrument.offsetBetweenReferenceAndWavelengthIDs
    ...
    def findPeaks(self):
        ...
        for element in self.elements:
            self.aperture.arcs[element] = self.aperture.extract(
                n=element, image=self.aperture.images[element], arc=True
            )
        ...
        for count, element in enumerate(self.elements):
            ...
            # identify my peaks
            xPeak, yPeak, xfiltered, yfiltered = oned.peaks(
                self.aperture.waxis,
                flux,
                plot=False,
                xsmooth=30,
    ...
    def guessMatches(self):
        ...
        for element in self.elements:
            ...
            theirPeaksOnMyPixels = np.array(
                [known["pixelguess"] for known in self.knownwavelengths[element]]
            )
            ...
            for i in range(len(myPeaks)):
                ...
                closest = np.sort(
                    np.nonzero(np.abs(distance) == np.min(np.abs(distance)))[0]
                )[0]
    ...
    def create(self, remake=False):
        ...
        while self.notconverged:
            ...
            # identify outliers
            limit = 1.48 * oned.mad(self.residuals[self.good]) * 4
    ...
    def plotWavelengthFit(self, interactive=True):
        ...
        self.speak(
            "{}, {}".format(
                self.pixelstowavelengths,
                self.pixelstowavelengths.domain,
                self.pixelstowavelengths.window,
            )
        )
        self.figcal = plt.figure("wavelength calibration", figsize=(15, 6), dpi=72)
        self.interactivewave = iplot(
            4, 1, height_ratios=[0.1, 0.4, 0.2, 0.2], hspace=0.1, bottom=0.15
        )
        ...
        self.ax_header.set_title(
            "Wavelength Calib. for Aperture"
        ...
        if self.justloaded:
            ...
            text += "[new!]"
        self.ax_header.text(0.025, 0.5, text, va="center", ha="left", fontsize=10)
        ...
        for e in self.elements:
            ok = np.nonzero(
                [e in self.waveids["name"][i] for i in range(len(self.waveids))]
            )[0]

            xvals = self.waveids["pixel"][ok]
            yvals = self.waveids["wavelength"][ok]

            self.ax_header.scatter(xvals, yvals, marker="o", color=colors[e], alpha=0.3)
            xfine = np.linspace(
                min(self.waveids["wavelength"]), max(self.waveids["wavelength"])
            )
            # self.ax_header.plot(self.waveids, xfine,
            #                    alpha=0.3)

        # plot the overlap of the lamp spectra
        scatterkw = dict(marker="o", linewidth=0, alpha=0.5, color=self.emissioncolor)
        for element in self.elements:
        ...
        for e in self.elements:
            for tw in self.knownwavelengths[e]:
                pix = tw["pixelguess"]
                self.ax_walign.axvline(pix, ymin=0.9, color=colors[e], alpha=0.5)

        # plot the calibration
        x = np.linspace(*oned.minmax(self.aperture.waxis), num=200)
        ...
        self.ax_wcal.scatter(
            self.pixel[self.good], self.wavelength[self.good], **scatterkw
        )
        self.ax_wres.scatter(
            self.pixel[self.good], self.residuals[self.good], **scatterkw
        )
        scatterkw["marker"] = "x"
        bad = self.good == False
        ...
        self.ax_wres.set_xlim(*oned.minmax(self.aperture.waxis))

        self.ax_walign.scatter(
            self.pixel[self.handpicked],
        ...
        performance = "using a {}-degree {}".format(
            self.polynomialdegree, self.pixelstowavelengths.__class__.__name__
        )
        performance += " the calibration has an RMS of {0:.2f}A".format(rms)
        performance += " with {0:.0f} good points".format(np.sum(self.good))
        performance += " from {:.0f} to {:.0f}".format(
            *oned.minmax(self.pixel[self.good])
        )
        self.ax_wres.text(
            0.98,
            0.05,
        ...
        self.ax_wres.set_ylim(
            *np.array([-1, 1]) * np.maximum(oned.mad(self.residuals[self.good]) * 10, 1)
        )
        plt.draw()
        ...
        options["w"] = dict(
            description="[w]rite the new calibration",
            function=self.saveandquit,
            requiresposition=False,
        )

        options["h"] = dict(
            description="match up a [h]elium line",
            function=self.see,
            requiresposition=True,
        )
        options["n"] = dict(
            description="match up a [n]eon line",
            function=self.see,
            requiresposition=True,
        )
        options["a"] = dict(
            description="match up a [a]rgon line",
            function=self.see,
            requiresposition=True,
        )

        options["d"] = dict(
            description="change the polynomial [d]egree",
            function=self.changedegree,
            requiresposition=True,
        )

        options["z"] = dict(
            description="[z]ap a previously matched line",
            function=self.zap,
            requiresposition=True,
        )

        options["u"] = dict(
            description="[u]ndo all changes made this session",
            function=self.undo,
            requiresposition=False,
        )

        options["r"] = dict(
            description="[r]estart from all defaults",
    ...
    def checkvalid(self, pressed):
        if pressed.xdata is None:
            self.speak(
                """you typed "{0}", but the window didn't return a coordinate, please try again!""".format(
                    pressed.key
                )
            )
            return False
    ...
    def see(self, pressed):
        ...
        pixel = match["mine"]["w"]
        self.speak(
            "you are e[X]cited about an indentifiable emission line at {0:.1}".format(
                pixel
            )
        )
        self.ax_walign.axvline(
            pixel, alpha=0.5, color=colors[match["theirs"]["element"]]
        )
        ...
        self.speak(
            "updating the guess for {0}A from {1} to {2}".format(
                wavematch["wavelength"], wavematch["pixelguess"], pixel
            )
        )
