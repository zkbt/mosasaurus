    ...
    try:
        ...
        for i, width in enumerate(self.trace.extractionwidths):
            ...
            if subtractsky:
                ...
                P_xl = self.trace.traceCenter(self.w) * sky_subtracted_image / f_standard.reshape(self.waxis.shape[0], 1)
                P_xl = np.maximum(P_xl, 0)
                P_xl /= np.sum(P_xl, 1).reshape(self.waxis.shape[0], 1)

                # STEP 6
                # revise variance estimates
                fP = P_xl*f_standard.reshape(self.waxis.shape[0], 1)
                V = fP + S
                ...
                f = np.sum(P_xl*(D-S)/V, 1) / np.sum((P_xl**2)/V, 1)
                self.extracted[width]['raw_counts_optext'] = f
                varf = np.sum(P_xl, 1) / np.sum((P_xl**2)/V, 1)
