...
ignorekw = dict(
    cmap=one2another("palevioletred", "palevioletred", alphatop=0.75, alphabottom=0.0),
...
class Aperture(Talker):
    ...
    def setup(self, x, y):
        ...
        self.ystart = np.maximum(y - blueward, 0).astype(int)
        self.yend = np.minimum(y + redward, self.instrument.ysize).astype(int)
        self.xstart = np.maximum(x - self.maskWidth, 0).astype(int)
        self.xend = np.minimum(x + self.maskWidth, self.instrument.xsize).astype(int)
        ...
        self.speak(
            "created a spectroscopic aperture at ({0:.1f}, {1:.1f})".format(
                self.x, self.y
            )
        )
    ...
    def createCalibStamps(self, visualize=True, interactive=True):
        ...
        calibfilename = os.path.join(
            self.directory, "calibStamps_{0}.npy".format(self.name)
        )
        try:
            self.images = np.load(calibfilename, allow_pickle=True)[
                ()
            ...
            interesting = (
                ["science", "badpixels"]
                + self.instrument.detectorcalibrations
            ...
            for k in interesting:
                if k != "badpixels":
                    self.images[k] = twod.interpolateOverBadPixels(
            ...
            try:
                self.images["RoughFlat"] = self.images["flat"] / np.median(
                    self.images["flat"], self.sindex
                ).reshape(self.waxis.shape[0], 1)
            except KeyError:
    ...
    def createTrace(self):
        ...
        self.images["RoughLSF"] = np.exp(
            -0.5
            * ((self.s - self.trace.traceCenter(self.w)) / self.trace.tracefitwidth)
            ** 2
        )
        # self.displayTrace()
    ...
    def createNormalizedFlat(self, visualize=True):
        ...
        normfilename = os.path.join(
            self.directory, "normFlat_{0}.pdf".format(self.name)
        )
        calibfilename = os.path.join(
            self.directory, "calibStamps_{0}.npy".format(self.name)
        )
        ...
        minarray, maxarray = np.zeros(self.waxis.shape[0]), np.zeros(
            self.waxis.shape[0]
        )
        ...
        self.images["NormalizedFlat"] = (
            self.images["flat"] * normmask
        ) / medfilt_1d.reshape(self.waxis.shape[0], 1)
        ...
        np.save(calibfilename, self.images)
        self.speak(
            "saved normalized flat to calibration stamp {0}".format(calibfilename)
        )
    ...
    def spatialCuts(self):
        ...
        filename = os.path.join(
            self.directory, "diagnosticCuts_{0}.pdf".format(self.name)
        )
        ...
        gs = plt.matplotlib.gridspec.GridSpec(
            5, 2, left=0.06, right=0.98, bottom=0.04, top=0.98, hspace=0.17, wspace=0.15
        )
        cuts = {}
        ...
        for p, pix in enumerate(pixelcutinds):
            ...
            cuts[pix].plot(self.saxis, profile, color="k", lw=2, alpha=0.8)
            ...
            for width in widths:
                ...
                sky = twod.polyInterpolate(
                    image, skymask == 0, order=order, visualize=False
                )

                cuts["stamp"].imshow(
                    extractmask,
                    cmap=one2another(
                        "aquamarine", "aquamarine", alphabottom=0.0, alphatop=1.0
                    ),
                    alpha=0.35,
                    aspect="auto",
                    extent=extent,
                ...
                cuts["stamp"].imshow(
                    skymask,
                    cmap=one2another(
                        "deepskyblue", "deepskyblue", alphabottom=0.0, alphatop=1.0
                    ),
                    alpha=0.3,
                    aspect="auto",
                    extent=extent,
                ...
                fit = np.polynomial.polynomial.polyfit(
                    self.saxis[ok], profile[ok], order
                )
                ...
                cuts[pix].plot(
                    self.saxis, poly(self.saxis), lw=3, color="deepskyblue", alpha=0.3
                )
                cuts[pix].fill_between(
                    self.saxis,
                    0,
                    maxcounts,
                    where=(notSkydata == 0),
    ...
    def extractedFilename(self):
        return os.path.join(
            self.directory, "extracted_{}.npy".format(self.exposureprefix)
        )
    ...
    def extract(self, n=0, image=None, subtractsky=True, arc=False, remake=False):
        ...
        try:
            ...
            for i, width in enumerate(self.trace.extractionwidths):
                ...
                self.intermediates[width]["extractMask"] = self.trace.extractionmask(
                    width
                )
                if arc:
                ...
                self.extracted[width]["intermediates"]["skyMask"] = self.intermediates[
                    width
                ]["skyMask"]
                self.extracted[width]["intermediates"]["extractMask"] = (
                    self.intermediates[width]["extractMask"]
                )
                    ...
                    try:
                        if arc == False:
                            self.intermediates[width]["smearedcosmics"] = (
                                self.mask.ccd.cosmicdiagnostic[
                                    self.xstart : self.xend
                                ].reshape(
                                    1, np.round(self.xend - self.xstart).astype(int)
                                )
                                * np.ones_like(image)
                                / (self.yend - self.ystart).astype(float)
                            )
                            ...
                            self.speak(
                                "the cosmic over-correction diagnostic is {0}".format(
                                    np.sum(self.extracted[width]["cosmicdiagnostic"])
                                )
                            )
                ...
                """
                    ...
                    self.intermediates[width]["sky"] = twod.polyInterpolate(
                    ...
                    self.extracted[width]["raw_counts"] = (
                        ...
                        * image
                        / self.images["NormalizedFlat"]
                    ).sum(self.sindex) - self.extracted[width]["sky"]
                ...
                """
                    ...
                    self.extracted[width]["raw_counts_optext"] = self.extracted[width][
                        "raw_counts"
                    ]
                    ...
                    self.extracted[width]["centroid"] = np.sum(
                        weights * self.s, axis=self.sindex
                    ) / np.sum(weights, axis=self.sindex)
                    ...
                    self.extracted[width]["peak"] = np.max(
                        self.intermediates[width]["extractMask"] * image, self.sindex
                    )
    ...
    def setupVisualization(self):
        self.thingstoplot = [
            "raw_counts"
        ]  # ['raw_counts_optext', 'sky', 'width',  'raw_counts']
        ...
        try:
            ...
            for i, width in enumerate(self.trace.extractionwidths):
                ...
                self.ax[width]["apertures"].set_ylim(*oned.minmax(self.saxis))
                ...
                for j, thing in enumerate(self.thingstoplot):
                    self.ax[width][thing] = plt.subplot(
                        gs[j, i], sharex=sharex, sharey=sharey[thing]
                    )
                    ...
                    if thing == "width":
                        self.ax[width][thing].set_ylim(
                            0, np.max(self.trace.extractionwidths) / 3.5
                        )
                    if thing == "centroid":
                        self.ax[width][thing].set_ylim(
                    ...
                    if thing == "raw_counts":
                        self.ax[width][thing].set_ylim(
                            0,
                            np.percentile(self.extracted[width]["raw_counts"], 99)
                            * 1.5,
                        )
                    if thing == "raw_counts_optext":
                        self.ax[width][thing].set_ylim(
                            0,
                            np.percentile(
                                self.extracted[width]["raw_counts_optext"], 99
                            )
                            * 1.5,
                        )
    ...
    def plotRectified(self, width, ax=None):
        ...
        ylim = oned.minmax(offsets)

        rectifiedw, rectifieds = np.meshgrid(self.waxis, np.arange(*ylim))
        rectifieds += self.trace.traceCenter(rectifiedw)

        indexw = np.interp(rectifiedw, self.waxis, np.arange(len(self.waxis))).astype(
            int
        )
        indexs = np.interp(rectifieds, self.saxis, np.arange(len(self.saxis))).astype(
            int
        )
        # indexw, indexs = wtoindex(rectifiedw).astype(int), stoindex(rectifieds).astype(int)
        ...
        rectified = self.intermediates[width]["subtracted"][
            indexw, indexs
        ]  # .reshape(rectifiedw.shape)
        ignore = (
            (
                self.intermediates[width]["skyMask"]
                + self.intermediates[width]["extractMask"]
            )
            == 0
        )[indexw, indexs]
        ...
        try:
            ...
            vmin = -nbackgrounds * np.min(
                [
                    1.48
                    * oned.mad(
                        self.intermediates[onewidth]["subtracted"][
                            self.intermediates[width]["skyMask"] > 0
                        ]
                    )
            ...
            ax.set_xlim(*oned.minmax(rectifiedw))
            ax.set_ylim(*ylim)
    ...
    def plotApertures(self, width, ax=None):
        ...
        try:
            ...
            vmin, vmax = (
                skylevel,
                peaklevel,
            )  # np.percentile(imscale(image), [1,99])
        ...
        for j, thing in enumerate(self.thingstoplot):
            if thing in self.extracted[width].keys():
                try:
                    ...
                    self.plotted[width][thing] = ax[thing].plot(
                        self.extracted["w"],
    ...
    def wavelengthcalibrate(self, n):
        ...
        try:
            self.extracted["wavelength"] = (
                self.wavelengthcalibrator.pixelstowavelengths(self.waxis)
            )
        except AttributeError:
    ...
    def addWavelengthCalibration(self, exposureprefix, remake=False, shift=False):
        ...
        if shift:
            ...
            mkdir(dir)
            self.supersampledFilename = os.path.join(
                dir, "stretchedsupersampled_{}.npy".format(self.exposureprefix)
            )
        else:
            dir = os.path.join(self.directory, "supersampled")
            mkdir(dir)
            self.supersampledFilename = os.path.join(
                dir, "supersampled_{}.npy".format(self.exposureprefix)
            )
    ...
    def interpolate(self, remake=False, shift=False):
        ...
        try:
            ...
            self.speak(
                "loaded supersampled spectrum from {0}".format(
            ...
            if shift:
                ...
                try:
                    ...
                    stretchfilename = os.path.join(
                        self.mask.reducer.extractionDirectory, "spectralstretch.npy"
                    )
            ...
            if self.visualize:
                ...
                try:
                    ...
                    self.figure_supersampled = plt.figure(
                        "interpolating spectra", figsize=(20, 10), dpi=60
                    )
                    ...
                    gs = plt.matplotlib.gridspec.GridSpec(
                        nkeys, napertures, hspace=0.1, wspace=0.05
                    )
                    self.plotted_supersampled = {}
            ...
            if shift:
                ...
                coefficients = (
                    self.stretches["stretch"][star][self.exposureprefix],
                    self.stretches["shift"][star][self.exposureprefix],
                )
                ...
                phrase = "dw = {:.4}x(w - {midpoint}){:+.4}".format(
                    *coefficients, midpoint=midpoint
                )
                self.speak(
                    "nudge wavelengths for {} by {}".format(self.exposureprefix, phrase)
                )
            else:
            ...
            try:
                ...
                doriginaldnew = resample_while_conserving_flux(
                    wavelength, np.ones_like(pixelnumber), commonwavelength
                )
                ...
                for j, width in enumerate(widths):
                    ...
                    yclosetonan = (
                        resample_while_conserving_flux(
                            wavelength, isnanoriginal, self.supersampled["wavelength"]
                        )
                        > 0
                    )
                    ...
                    if key in self.additivekeys:
                        # resample_while_conserving_flux by default gives a sum
                        ...
                        self.supersampled[combinedkey] = (
                            ysupersampled / self.supersampled["fractionofapixel"]
                        )
                    else:
                        self.speak(
                            "Yikes! It's not clear if {} is an additive or intrinsic quantity!".format(
                                key
                            )
                        )
                    ...
                    if self.visualize:
                        if key in self.additivekeys:
                            ontheoriginalpixels = (
                                self.supersampled[combinedkey]
                                / self.supersampled["fractionofapixel"]
                            )
                        elif key in self.intrinsickeys:
                        ...
                        try:
                            ...
                            plot_extracted, plot_supersampled = (
                                self.plotted_supersampled[combinedkey]
                            )
                            plot_extracted[0].set_data(
                                wavelength, self.extracted[width][key]
                            )
                            plot_supersampled[0].set_data(
                                self.supersampled["wavelength"], ontheoriginalpixels
                            )
                        except KeyError:
                            ...
                            plot_extracted = self.ax_supersampled[combinedkey].plot(
                                wavelength,
                                self.extracted[width][key],
                                color="black",
                                alpha=0.5,
                            )
                            plot_supersampled = self.ax_supersampled[combinedkey].plot(
                                self.supersampled["wavelength"],
                                ontheoriginalpixels,
                                color="red",
                                alpha=0.5,
                            ...
                            try:
                                ...
                                lims = np.percentile(
                                    ontheoriginalpixels[ok], [5, 95]
                                )  # np.nanmin(), np.nanmax(self.extracted[width][key])
                                ...
                                if "centroid" not in key:
                                    self.limits_supersampled[combinedkey][0] = (
                                        np.maximum(
                                            0, self.limits_supersampled[combinedkey][0]
                                        )
                                    )
                            ...
                            if key == self.keys[-1]:
                                ...
                                plt.setp(
                                    self.ax_supersampled[combinedkey].get_xticklabels(),
                                    visible=False,
                                )
                            if width != widths[0]:
                                plt.setp(
                                    self.ax_supersampled[combinedkey].get_yticklabels(),
                                    visible=False,
                                )
            ...
            if self.visualize:
                # plt.show()
                # plt.draw()
                supersamplingPlotFilename = self.supersampledFilename.replace(
                    "npy", "pdf"
                )
            ...
            self.speak(
                "saved supersampled spectrum to {0}".format(self.supersampledFilename)
            )
