...
class SquishableCube(Talker):
    ...
    def corrected(self, key="raw_counts"):
        ...
        star = self.target
        self.speak(
            "calculating [{}] for [{}], corrected by the mega-calibrator".format(
                key, star
            )
        )
    ...
    def wavelengthed(self, key="raw_counts", star=None):
        ...
        # normalize along the wavelength axis
        self.speak(
            "calculating [{}] for [{}], normalized by its median spectrum".format(
                key, star
            )
        )
    ...
    def timed(self, key="raw_counts", star=None):
        ...
        # normalize along the wavelength axis
        self.speak(
            "calculating [{}] for [{}], normalized by its median time series".format(
                key, star
            )
        )
        z = self.cubes[key][star]
    ...
    def wavelengthedtimed(self, key="raw_counts", star=None):
        ...
        # normalize along the wavelength axis
        self.speak(
            "calculating [{}] for [{}], normalized by its median spectrum, and then by its median time series".format(
                key, star
            )
        )
        z = self.cubes[key][star]
    ...
    def imageCube(self, keys=None, stars=None, remake=False):
        ...
        for option, description in options.items():
            ...
            plt.suptitle(
                "{}, [{}shifted]".format(
                    description, {True: "", False: "un"}[self.shift]
                )
            )
    ...
    def markBad(self):
        ...
        try:
            ...
            for star in self.stars:
                ...
                self.speak(
                    "out of {} pixels...".format(
                        self.numberofwavelengths * self.numberoftimes
                    )
                )
                ...
                smoother = np.convolve(
                    haswidth, np.ones(buffer).astype(float) / buffer, mode="same"
                )
                ...
                oned = np.ones_like(unsaturated).astype(bool)
            ...
            """
                ...
                pixelfraction = (
                    np.sum(self.cubes["ok"][star] > 0).astype(float)
                    / self.numberofwavelengths
                    / self.numberoftimes
                )
                exposurefraction = (
                    np.sum(np.max(self.cubes["ok"][star], 1) > 0) / self.numberoftimes
                )
                wavelengthfraction = (
                    np.sum(np.max(self.cubes["ok"][star], 0) > 0)
                    / self.numberofwavelengths
                )

                self.speak(
                    "For {}...,\n   {:.3%} of pixels are OK\n   {:.3%} of exposures are at least partially OK\n   {:.3%} of wavelengths are at least partially OK".format(
                        star, pixelfraction, exposurefraction, wavelengthfraction
                    )
                )
    ...
    def squish(self, binning=50, remake=False):
        ...
        self.binnedfilename = os.path.join(
            self.binneddirectory, self.binlabel + "cube.npy"
        )
        ...
        if True:
            ...
            n = int(len(originalwavelength) / binning)

            self.binned_cubes = {}
            self.binned_spectral = {}
            self.binned_cubes["ok"] = {}
            for star in self.stars:
                self.binned_cubes["ok"][star] = np.ones(
                    (self.numberoftimes, int(self.numberofwavelengths / binning))
                )
            ...
            for k in ["raw_counts", "sky", "centroid", "width", "peak"]:
                ...
                for star in self.stars:
                    self.binned_cubes[k][star] = np.zeros(
                        (self.numberoftimes, int(self.numberofwavelengths / binning))
                    )
                    for i in tqdm(range(self.numberoftimes)):
                        ...
                        ok = self.cubes["ok"][star][i, :].astype(float)

                        # make a weighted resample
                        binned_weighted = resample_while_conserving_flux(
                            originalwavelength, y * ok, centers
                        )

                        # what were the weights?
                        binned_weights = resample_while_conserving_flux(
                            originalwavelength, ok, centers
                        )

                        # what would the weights be if everything were perfect?
                        binned_ones = resample_while_conserving_flux(
                        ...
                        self.binned_cubes["ok"][star][i, :] = (
                            binned_weights / binned_ones
                        )  # *= binned_weights > 0
            ...
            self.binned_spectral["fractionofapixel"] = resample_while_conserving_flux(
                originalwavelength, self.spectral["fractionofapixel"], centers
            )

            for fraction in np.linspace(0, 1, 11):
                self.speak(
                    "{} points are <={} okay".format(
                        np.sum(self.binned_cubes["ok"][self.target] <= fraction),
                        fraction,
                    )
                )
    ...
    def export(self):
        ...
        for thing in tosave.keys():
            self.speak(
                "  including [{0}] in the binned saved cube structure".format(thing)
            )
    ...
    def zapCosmics(self, wavelengthbin=10, threshold=5, plot=True, remake=False):
        ...
        try:
            ...
            binned = np.mean(
                self.corrected().reshape(
                    ntimes, nwavelengths / wavelengthbin, wavelengthbin
                ),
                2,
            )
            ...
            badoriginalshape = (bad[:, :, np.newaxis] * np.ones(wavelengthbin)).reshape(
                (ntimes, nwavelengths)
            )
            ...
            if plot:
                ...
                self.visualizer.explore(
                    key="corrected", vmin=0.97, vmax=1.01, interpolation="nearest"
                )
                self.visualizer.overlayQuality()
                plt.savefig(filename)
                self.speak(
                    "saved plot of cosmic identifications to {}".format(filename)
                )
...
class BinnedCube(SquishableCube):
    def __init__(self, filename=None, **kwargs):
        ...
        self.load()
        self.binned = True
        self.binsize = int(filename.split("binby")[-1].split("cube")[0])
    ...
    def makeLCs(self):
        ...
        noisecountstarget = binsize * (
            self.cubes["raw_counts"][self.target] + self.cubes["sky"][self.target]
        )
        ...
        for c in self.comparisons:
            noisecountscomparisons += binsize * (
                self.cubes["raw_counts"][c] + self.cubes["sky"][c]
            )
            signalcountscomparisons += binsize * (self.cubes["raw_counts"][c])
        fractionalcomparisonsuncertainty = (
            np.sqrt(noisecountscomparisons) / signalcountscomparisons
        )

        uncertainty = np.sqrt(
            fractionaltargetuncertainty**2 + fractionalcomparisonsuncertainty**2
        )
        ...
        for i, w in enumerate(wavelengths):
            ...
            for key in ["width", "centroid", "shift"]:
                try:
                    ...
                    for comparison in self.comparisons:
                        lc["{0}_{1}".format(key, starname(comparison))] = self.squares[
                            key
                        ][comparison]
                except KeyError:
            ...
            for key in ["sky", "peak"]:
                ...
                for comparison in self.comparisons:
                    lc["{0}_{1}".format(key, starname(comparison))] = self.cubes[key][
                        comparison
                    ][:, i]
            ...
            for key in ["width", "centroid"]:
                lc["d{0}_target".format(key)] = (
                    self.cubes[key][self.target][:, i] - lc["{0}_target".format(key)]
                )
                for comparison in self.comparisons:
                    lc["d{0}_{1}".format(key, starname(comparison))] = (
                        self.cubes[key][comparison][:, i]
                        - lc["{0}_{1}".format(key, starname(comparison))]
                    )
            ...
            lcfilename = os.path.join(
                lcdirectory,
                "{0:05d}to{1:05d}.lightcurve".format(int(left), int(right)),
            )

            table.write(
                lcfilename, format="ascii.fixed_width", bookend=False, overwrite=True
            )
