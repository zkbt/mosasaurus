...
def iraf2python(s):
    ...
    bottom, top = np.array(rows.split(":")).astype(int) - np.array([1, 0])
    left, right = np.array(cols.split(":")).astype(int) - np.array([1, 0])
...
class Spectrograph(Talker):
    ...
    def setupDirectories(
        ...
        self.speak(
            "expecting to find raw data in nightly directories inside {}".format(
                self.dataDirectory
            )
        )

        # working directory is where in-progress files + results are created
        self.spectrographWorkingDirectory = os.path.join(
            self.baseDirectory, workingDirectory
        )
        mkdir(self.spectrographWorkingDirectory)
        self.speak(
            "all working data will be stored in {}".format(
                self.spectrographWorkingDirectory
            )
        )

        # extraction directory is where extracted spectra land
        self.spectrographExtractionDirectory = os.path.join(
            self.baseDirectory, extractionDirectory
        )
        ...
        self.workingDirectory = os.path.join(
            self.spectrographWorkingDirectory, "{}-{}".format(self.name, self.disperser)
        )
        mkdir(self.workingDirectory)

        self.extractionDirectory = os.path.join(
            self.spectrographExtractionDirectory, self.disperser
        )
