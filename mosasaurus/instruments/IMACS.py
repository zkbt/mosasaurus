...
class IMACS(Spectrograph):
    ...
    basicpattern = (
        "ift*c8.fits"  # for now we will just extract chip 8; stitching is hard
    )
    ...
    def findBiases(self, night):
        ...
        match = night.find(
            wordstolookfor=self.wordstosearchfor["bias"], placetolook=self.keytosearch
        )
        return match
    ...
    def findHe(self, night):
        ...
        match = night.find(
            wordstolookfor=self.wordstosearchfor["He"], placetolook=self.keytosearch
        )
        return match
    ...
    def findNe(self, night):
        ...
        match = night.find(
            wordstolookfor=self.wordstosearchfor["Ne"], placetolook=self.keytosearch
        )
        return match
    ...
    def findAr(self, night):
        ...
        match = night.find(
            wordstolookfor=self.wordstosearchfor["Ar"], placetolook=self.keytosearch
        )
        return match
    ...
    def __init__(self, grism="vph-red"):
        ...
        self.observatory = coord.EarthLocation.from_geodetic(
            lon=coord.Angle("-70° 41′ 33.36″"),
            lat=coord.Latitude("-29° 0′ 52.56″"),
            height=2380 * u.m,
        )
    ...
    def setupDetector(self):
        ...
        self.detectorcalibrations = [
            "flat"
        ]  # for some data sets where I didn't take biases or flats (you shouldn't need them for IMACS)
    ...
    def setupDisperser(self):
        ...
        self.offsetBetweenReferenceAndWavelengthIDs = 0.0
        ...
        self.disperserDirectory = os.path.join(
            mosasaurusdirectory, "data/", self.name + "/", self.disperser + "/"
        )
        self.wavelength2pixelsFile = os.path.join(
            self.disperserDirectory,
            "{0}_wavelength_identifications.txt".format(self.grism),
        )  # at this stage this .txt file is NOT the correct one for IMACS

        self.wavelengthsFile = os.path.join(self.disperserDirectory, "HeNeAr.txt")
    ...
    def setupDirectories(
        ...
        self.dataDirectory = os.path.join(self.baseDirectory, dataDirectory)
        mkdir(self.dataDirectory)

        # working directory is where in-progress files + results are created
        self.workingDirectory = os.path.join(self.baseDirectory, workingDirectory)
        mkdir(self.workingDirectory)

        # extraction directory is where extracted spectra land
        self.extractionDirectory = os.path.join(self.baseDirectory, extractionDirectory)
    ...
    def extractMidExposureTimes(self, headers):
        ...
        timestrings = [
            "{0} {1}".format(row["ut-date"], row["ut-time"]) for row in headers
        ]

        # calculate a JD from these times (and adding half the exposure time, assuming ut-time is the start of the exposure)
        starttimes = astropy.time.Time(
            timestrings, format="iso", scale="utc", location=self.observatory
        )
    ...
    def stitchChips(self, listOfChips):
        ...
        bottomrow = np.hstack(
            (
                np.flipud(listOfChips[6]),
                np.flipud(listOfChips[7]),
                np.flipud(listOfChips[4]),
                np.flipud(listOfChips[5]),
            )
        )
        toprow = np.hstack(
            (
                np.fliplr(listOfChips[3]),
                np.fliplr(listOfChips[2]),
                np.fliplr(listOfChips[1]),
                np.fliplr(listOfChips[0]),
            )
        )
