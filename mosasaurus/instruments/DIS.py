...
class DIS(Spectrograph):
    ...
    def __init__(self, grating="R300"):
        ...
        self.observatory = coord.EarthLocation.from_geodetic(
            -105.82 * u.deg, 32.78 * u.deg, 2798.0 * u.m
        )
    ...
    def extractMidExposureTimes(self, headers):
        ...
        starttimes = astropy.time.Time(
            timestrings, format="isot", scale="utc", location=self.observatory
        )
    ...
    def loadSingleCCD(self, filenames):
        ...
        trimmed = self.data[dbottom:dtop, dleft:dright].astype(float)
