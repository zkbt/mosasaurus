...
class LDSS3C(Spectrograph):
    ...
    def __init__(self, grism="vph-red", binning=2):
    ...
    def setupDetector(self):
        ...
        self.gains = np.array(
            [2.84, 2.52]
        )  # gain from 2021, turbo read out mode (March 2021)
    ...
    def setupDisperser(self):
        ...
        self.disperserDataDirectory = os.path.join(
            mosasaurusdirectory, "data/", self.name + "/", self.disperser + "/"
        )
        self.wavelength2pixelsFile = os.path.join(
            self.disperserDataDirectory,
            "{0}_wavelength_identifications.txt".format(self.grism),
        )

        self.wavelengthsFile = os.path.join(self.disperserDataDirectory, "HeNeAr.txt")
    ...
    def loadOverscanTrimHalfCCD(self, filename):
        ...
        print(self.databottom, self.datatop, self.dataleft, self.dataright)
        # estimate the 1D bias (and drawdown, etc...) from the overscan
        biasEstimate = np.nanmedian(goodBias, axis=1)
    ...
    def loadSingleCCD(self, filenames):
        """
        Load a LDSS3C image; subtract and trim its overscan.
        ...
        # load the chips
        c_data = [self.loadOverscanTrimHalfCCD(f)[0] for f in filenames]
        c_header = [self.loadOverscanTrimHalfCCD(f)[1] for f in filenames]
