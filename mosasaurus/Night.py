...
class Night(Talker):
    ...
    def createNightlyLog(self, remake=False):
        ...
        self.speak(
            '{} contains {} files matching "{}"'.format(
                self.dataDirectory, len(self.filenames), pattern
            )
        )

        # load or create a nightly obseravtion log
        self.logFilename = os.path.join(
            self.directory, "nightly_log_{}.txt".format(self.name)
        )
        ...
        try:
            ...
            for file in self.filenames:
                self.speak(
                    "Loading header information from {}.".format(
    ...
    def createSummaryLog(self, remake=False):
        ...
        self.summaryFilename = os.path.join(
            self.directory, "nightly_summary_{}.txt".format(self.name)
        )
        try:
            assert remake == False
            ...
            self.speak(
                "Loaded a condensed summary from {}.".format(self.summaryFilename)
            )
        except (AssertionError, IOError):
            ...
            bad = (self.log[column] == "") | np.array(
                [type(c) == None for c in self.log[column]]
            )
            self.log[column][bad] = "???"
            ...
            def glom(a):
                try:
                    return "{:.3}\pm{:.3}".format(
                        np.nanmedian(a), 1.486 * np.median(np.abs(a - np.median(a)))
                    )
                except TypeError:
                    s = " + ".join(
                        ['("{}")x{}'.format(u, np.sum(a == u)) for u in np.unique(a)]
                    )
                    if len(s) > 30:
            ...
            self.speak(
                "wrote the aggregated summary for {} to {}".format(
                    self.name, self.summaryFilename
                )
            )
    ...
    def find(self, wordstolookfor, placetolook, wordstoavoid=[]):
        ...
        match = np.zeros(len(self.log)).astype(bool)

        # find whether the wordstolookfor are seen anywhere in the placetolook
        for w in wordstolookfor:
            thismatch = np.array(
                [w.lower() in word.lower() for word in self.log[placetolook]]
            )

            # reject those bad ones
            for bad in wordstoavoid:
                isbad = np.array(
                    [bad.lower() in word.lower() for word in self.log[placetolook]]
                )
                thismatch = thismatch & (isbad == False)

            self.speak(
                '{} elements in "{}" contained "{}"'.format(
