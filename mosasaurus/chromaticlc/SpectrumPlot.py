...
class Gradient(Color):
    def __init__(self, bottom="indigo", top="orange", **kwargs):
        Color.__init__(self, **kwargs)
        self.cmap = one2another(bottom, top)
        ...
        for i in range(self.npanels):
            ...
            if i == (self.npanels - 1):
                ...
                try:
                    ...
                    self.xlim = [
                        (spectrum.bins[0].left - spectrum.binsize / 2) / spectrum.unit,
                        (spectrum.bins[-1].right + spectrum.binsize / 2)
                        / spectrum.unit,
                    ]
                ax.set_xlim(*self.xlim)
...
class StellarSpectrum(Plottable):
    ...
    def plot(self, ax):
        ...
        for s in spectrum.spectra:
            wavelength, stellarSpectrum = np.load(
                s.obs.extractionDirectory + "medianSpectrum.npy"
            )
            ax.plot(
                wavelength / spectrum.unit,
                stellarSpectrum * spectrum.unit,
                linewidth=3,
                alpha=0.5,
                color="black",
            )
...
class FittedParameter(Plottable):
    ...
    def plot(self, ax):
        ...
        spectrum = self.parent.spectrum
        key = self.key
        self.wavelengths, self.parameter, self.uncertainty, epochs = (
            spectrum.spectrumOf(key)
        )
