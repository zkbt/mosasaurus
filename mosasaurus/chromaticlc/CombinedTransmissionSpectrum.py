...
class CombinedObs(Talker):
    ...
    def __init__(self, listofobs):
        ...
        self.workingDirectory = (
            obs.baseDirectory
            + "working/combinationof{0}observations/".format(len(listofobs))
        )
        mkdir(self.workingDirectory)
        self.extractionDirectory = (
            self.workingDirectory + obs.extractionDirectory.split("/")[-2] + "/"
        )
...
class CombinedTransmissionSpectrum(TransmissionSpectrum):
    ...
    def __init__(
        ...
        for b in np.unique(binoptions):
            left, right = np.array(b.split("nm")[0].split("to")).astype(int) * 10
    ...
    def fitMonochromatic(
        ...
        **kw
        ...
        try:
            ...
            raise ValueError("likelihoodtype is not in {0}".format(likelihoodtypes))
        ...
        for i, orig in enumerate(self.archiveoftlcs[w]):
            ...
            synthesizerdirectory = "{0}{1}/".format(
                self.fitdirectory, self.bins[self.w2bin(w)][0].identifier
            )
            mkdir(synthesizerdirectory)

            # assign an epoch to the TLC
            tlc = orig.splitIntoEpochs(
                planet,
                newdirectory=synthesizerdirectory,
                phaseofinterest=self.phaseofinterest,
            )[0]
            ...
            instrument = transit.Instrument(
                tlc=tlc, gplna=-5, gplntau=-5, **initialConditions.instrumentkw
            )
            ...
            try:
                instrument.shift_target_tothe1.float(
                    value=0.002, limits=[-0.005, 0.005]
                )
            except AttributeError:
        ...
        if mcmc:
            self.synthesizer = transit.MCMC(
                tlcs=tlcs, directory=synthesizerdirectory, likelihoodtype=likelihoodtype
            )
        else:
        ...
        if plot:
            transit.MultiplexPlot(
                self.synthesizer.tlcs,
                transit.DiagnosticsPlots,
                wobbly=True,
                everything=ploteverything,
                figsize=(30, 10),
                dpi=72,
                mintimespan=mintimespan,
                binsize=15 / 60.0 / 24.0,
            )
    ...
    def combine(self):
        ...
        for i in range(len(self.wavelengths)):
            ...
            for k in f2combine.keys():
                ...
                self.fitted[k][i] = np.sum(f2combine[k] / u2combine[k] ** 2) / np.sum(
                    1.0 / u2combine[k] ** 2
                )
                self.chisq[k][i] = np.sum(
                    (f2combine[k] - self.fitted[k][i]) ** 2 / u2combine[k] ** 2
                )
                self.dof[k][i] = max(len(f2combine[k]) - 1.0, 1)
                rescaling = max(np.sqrt(self.chisq[k][i] / self.dof[k][i]), 1)

                self.uncertainty[k][i] = (
                    1.0 / np.sqrt(np.sum(1.0 / u2combine[k] ** 2)) * rescaling
                )
    ...
    def mask(self, value):
        ...
        for spectrum in self.spectra:
            self.speak(
                "setting mask for {spectrum} to {mask}".format(
                    spectrum=spectrum, mask=value
                )
            )
    ...
    def performance(self):
        ...
        for w in wavelengths:
            ...
            for i, tlc in enumerate(listoftlcs):
                ...
                print(
                    "   {0} {1:>6.0f} {2:>6.0f}".format(
                        tlc.name.split(",")[1],
                        np.mean(tlc.effective_uncertainty[tlc.ok]) * 1e6,
                        np.std(tlc.residuals()[tlc.ok]) * 1e6,
                    )
                )
    ...
    def label(self, value):
        ...
        for spectrum in self.spectra:
            self.speak(
                "setting label for {spectrum} to {label}".format(
                    spectrum=spectrum, label=value
                )
            )
