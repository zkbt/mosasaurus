...
class TS(Talker):
    ...
    def binningdirectory(self):
        bd = self.obs.extractionDirectory + "binby{binsize:05.0f}/".format(
    ...
    def loadLCs(self):
        try:
            ...
            lcDirectory = (
                self.obs.extractionDirectory + "lc_binby{0}".format(self.binsize) + "/"
            )
        ...
        for trimmed in chunks:
            ...
            self.speak(
                "creating a bin between {left} and {right}".format(
    ...
    def createMask(self):
        ...
        kw = dict(
            cmap="gray", interpolation="nearest", aspect="auto", vmin=None, vmax=None
        )
        ...
        while keepgoing:
            ...
            while unreasonableanswer:
                ...
                if "a" in answer:
                    ...
                    mask[bottom:top, left:right] = (
                        mask[bottom:top, left:right] | self.bins[0].tlc.flags["custom"]
                    )
    ...
    def fit(self, planet, star, instrument, plot=True):
        ...
        self.ld = LD.LD(
            temperature=star.temperature.value,
            gravity=star.logg.value,
            metallicity=star.metallicity.value,
            directory=self.binningdirectory,
            plot=True,
        )
    ...
    def setupSuperPlot(self):
        ...
        gs = plt.matplotlib.gridspec.GridSpec(
            3, 1, wspace=0, hspace=0.05, height_ratios=[1, 2, 2]
        )
        ...
        self.ax_ts.set_xlim(
            (self.bins[0].left - self.binsize / 2) / self.unit,
            (self.bins[-1].right + self.binsize / 2) / self.unit,
        )
    ...
    def plot(self):
        ...
        # plot the spectrum
        try:
            wavelength, spectrum = np.load(
                self.obs.extractionDirectory + "medianSpectrum.npy"
            )
            self.ax_spectrum.plot(
                wavelength / self.unit,
                spectrum * self.unit,
                linewidth=3,
                alpha=0.5,
                color="black",
        ...
        for i in np.arange(len(self.bins)):
            ...
            kw = {
                "marker": ".",
                "color": craftroom.color.nm2rgb(
                    [bin.left / self.unit, bin.right / self.unit]
                ),
                "alpha": 0.25,
                "linewidth": 0,
                "marker": "o",
            ...
            self.ax_lc.plot(
                normalize(bin.tlc.corrected()[bin.tlc.bad == False])
                + bin.wavelength / self.unit,
                bin.tlc.timefrommidtransit()[bin.tlc.bad == False],
                **kw
            )
    ...
    def fitFlexible(
        self, filename="wasp94_140805.obs", binsize=100, remake=False, plot=False
    ):
        plt.ion()
...
def determineParameters(filename="wasp94_140805.obs", binsize=500):
    ...
    for k in keys:
        ...
        dict[k + "_uncertainty"] = [
            bin.tm.planet.__dict__[k].uncertainty for bin in ts.bins
        ]

    self.speak("")
    self.speak(
        "The median values of the {binsize} angstrom fits for {filename} are:".format(
            binsize=binsize, filename=filename
        )
    )
