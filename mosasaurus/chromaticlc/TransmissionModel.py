...
class TransmissionModel(Talker):
    ...
    def propagate(self):
        ...
        self.original_deltaradiusinrj = self.original_radiusinrj - np.min(
            self.original_radiusinrj[optical]
        )
        self.scale_factor = self.scaleheight / self.original_scaleheight

        self.nscaleheights = (
            self.original_deltaradiusinrj * u.Rjupiter / self.original_scaleheight
        )
        ...
        self.deltadepth = (
            ...
            * self.radius
            * u.Rearth
            * self.deltaradiusinrj
            * u.Rjupiter
            / (self.stellar_radius * u.Rsun) ** 2
        )
    ...
    def binas(self, spectrum):
        ...
        for i in range(len(spectrum.wavelength)):
            ...
            ok = (self.wavelength / 10.0 >= spectrum.left[i]) * (
                self.wavelength / 10.0 < spectrum.right[i]
            )
    ...
    def binned(self, binwidth=100):
        return oned.binto(self.wavelength, self.nscaleheights, binwidth)
...
class Fortney(TransmissionModel):
    ...
    def __init__(
        self, planet="WASP94Ab", modelpath="/Users/zkbt/Dropbox/fortneyhotjupiter/"
    ):

        TransmissionModel.__init__(self, planet=planet)

        self.original_scaleheight = (
            u.k_B
            * self.original_teq
            / u.mp
            / self.original_mu
            / self.original_surfacegravity
        )
...
class Morley(TransmissionModel):
    def __init__(
        self, planet="WASP94Ab", modelpath="/Users/zkbt/Dropbox/morleywasp94/"
    ):
