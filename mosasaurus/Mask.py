...
class Mask(Talker):
    ...
    def pickStars(self):
        ...
        extractionCentersFilename = os.path.join(
            self.reducer.extractionDirectory, "extractionCenters.txt"
        )
        if os.path.exists(extractionCentersFilename):
            self.speak(
                "Extraction centers were already defined; loading them from {0}".format(
                    extractionCentersFilename
                )
            )
            x, y = np.transpose(np.loadtxt(extractionCentersFilename))
        else:
            self.speak(
                "Please should pick the stars you're interested in, then [q]uit."
            )
            ...
            self.loupe.options["a"] = dict(
                description="[a]dd an extraction center",
                function=self.addExtractionCenter,
                requiresposition=True,
            )

            # add in an option to pull out an extraction center
            self.loupe.options["r"] = dict(
                description="[r]estart, clearing all centers",
    ...
    def resetExtractionCenters(self, pressed):
        ...
        try:
            ...
            self.centersplot = self.loupe.ax["2d"].plot(
                self.xcenters,
                self.ycenters,
                linewidth=0,
                marker="x",
    ...
    def addExtractionCenter(self, pressed):
        ...
        self.xcenters.append(x.astype(int))
        self.ycenters.append(y.astype(int))
    ...
    def setup(self, visualize=True, remake=False):
        ...
        try:
            ...
            if (
                os.path.exists(self.reducer.extractionDirectory + "apertures.npy")
                and remake == False
            ):
            ...
            filename = os.path.join(
                self.reducer.extractionDirectory, "genericfinderchart.pdf"
            )
            if visualize and (os.path.exists(filename) == False):
                ...
                for a in self.apertures:
                    ...
                    self.loupe.ax["2d"].text(
                        a.y,
                        a.x,
                        "   {:03.0f},{:03.0f}".format(a.x, a.y),
                        color="black",
                        alpha=0.5,
                        ha="left",
                        va="center",
    ...
    def extractStars(self, n, remake=False):
        ...
        # provide update
        self.speak(
            "extracting {0} spectra from image #{1}".format(len(self.apertures), n)
        )
    ...
    def extractEverything(self, remake=False):
        ...
        self.speak(
            "adding wavelength calibrations to all spectra (if not already done)"
        )
